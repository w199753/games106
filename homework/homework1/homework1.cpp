...
public:
	...
	*/
		...
		if (inputNode.mesh > -1) {
			...
			for (size_t i = 0; i < mesh.primitives.size(); i++) {
				...
				// Vertices
				{
					...
					//std::cout << mesh.primitives.size()<<"   "<< vertexCount << std::endl;
	...
	void UpdateVertexBuffer(const tinygltf::Node& inputNode, const tinygltf::Model& input, VulkanglTFModel::Node* node, uint32_t nodeIndex, std::vector<VulkanglTFModel::Vertex>& vertexBuffer)
	{
		...
		//	}
		//}
		std::cout << "fzy log:" << nodes.size()<<"   "<< inputNode.name << "   " << node->children.size() << std::endl;
		for (int i = 0; i < nodes[0]->children.size(); i++)
		{
			if (nodes[i]->children.size() > 0)
			{
				UpdateVertexBuffer(input.nodes[inputNode.children[i]], input, nodes[i]->children[i], inputNode.children[i], vertexBuffer);
			}
	...
	*/
	...
	// POI: Traverse the node hierarchy to the top-most parent to get the local matrix of the given node
		...
		void VulkanglTFModel::updateAnimation(const tinygltf::Model & model, float deltaTime)
		{
			...
			{
				/*std::cout << channel.node->index << std::endl;*/
				AnimationSampler& sampler = animation.samplers[channel.samplerIndex];
				//std::cout<<"fzy " << animation.samplers.size() << std::endl;
				...
				//	glTFModel.UpdateVertexBuffer(node, model, nullptr, scene.nodes[i], indexBuffer, vertexBuffer);
				//}
			}
		}
	...
	public:
		...
		void loadglTFFile(std::string filename)
		{
			...
			if (fileLoaded) {
				...
				std::cout << "fzy test:" << scene.nodes.size() << "    " << glTFInput.nodes.size() << std::endl;
				for (size_t i = 0; i < scene.nodes.size(); i++) {
					const tinygltf::Node node = glTFInput.nodes[scene.nodes[i]];
					glTFModel.loadNode(node, glTFInput, nullptr, scene.nodes[i], indexBuffer, vertexBuffer);
					glTFModel.UpdateVertexBuffer(node, glTFInput, glTFModel.nodes[0], scene.nodes[i], vertexBuffer);
				}
		...
		virtual void render()
		{
			...
			if (!paused)
			{
				glTFModel.updateAnimation(glTFInput, frameTimer);
			}
		}
